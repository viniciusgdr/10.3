...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const { text, extendedText, contact, location, chatId, isGroupMsg, id, liveLocation, image, video, sticker, document, audio, product } = MessageType
			...
			if (isGroupMsg && GroupLinkDetector && !isGroupAdmins && !isOwner){
				if (chats.match((https://chat.whatsapp.com)/gi)) {
					...
					if (!check) {
						...
						client.sendMessage(from, '*[GROUP LINK DETECTOR]*\n manda mta merda, só irei tirar do grp pq mando link seu filho da putaz\n na proxima vez, vá mandar link pra sua vó! :(', id).then(() => {
			...
			switch (command) {
				...
				case 'loli':
					...
					break
				case 'antilink':
					if (!isGroupMsg) return client.sendMessage(from, 'TEM QUE TAR EM GRUPO FILHO DA PUTA', id)
					if (!isGroupAdmins) return client.sendMessage(from, 'precisas ser adm companheiro', id)
					if (!isBotGroupAdmins) return client.sendMessage(from, 'Anti link foi Ativado', id)
					if (args[0] == 'on') {
						var cek = antilink.includes(chatId);
						if(cek){
							return client.sendMessage(from, '*Anti Group Link Detector*', id) //if number already exists on database
						} else {
							antilink.push(chatId)
							fs.writeFileSync('./lib/helper/antilink.json', JSON.stringify(antilink))
							client.sendMessage(from, '*[Anti Group Link]* telah di aktifkan\nSetiap member grup yang mengirim pesan mengandung link grup akan di kick oleh bot!', id)
						}
					} else if (args[0] == 'off') {
						var cek = antilink.includes(chatId);
						if(!cek){
							return client.sendMessage(from, '*Anti Group Link Detector* sudah non-aktif di grup ini', id) //if number already exists on database
							} else {
							let nixx = antilink.indexOf(chatId)
							antilink.splice(nixx, 1)
							fs.writeFileSync('./lib/helper/antilink.json', JSON.stringify(antilink))
							client.sendMessage(from, '*[Anti Group Link]* telah di nonaktifkan\n', id)
						}
					} else {
						client.sendMessage(from, `pilih on / off\n\n*[Anti Group Link]*\nSetiap member grup yang mengirim pesan mengandung link grup akan di kick oleh bot!`, id)
					}
					break  
				case 'promover':
